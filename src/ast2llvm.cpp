...
Func_local* ast2llvmFunc(aA_fnDef f)
{
    ...
    for (const auto &decl : f->fnDecl->paramDecl->varDecls) {
        if (decl->kind == A_varDeclScalarKind) {
            if (decl->u.declScalar->type->type == A_structTypeKind) {
                string structname = *decl->u.declScalar->type->u.structType;
                Temp_temp* def = Temp_newtemp_struct_ptr(0, structname);
                def->varname = *decl->u.declScalar->id;
                args.push_back(def);
                localVarMap.emplace(def->varname, def);
            } else {
                Temp_temp* def = Temp_newtemp_int();
                def->varname = *decl->u.declScalar->id;
                args.push_back(def);
                Temp_temp* p = Temp_newtemp_int_ptr(0);
                emit_irs.emplace_back(L_Alloca(AS_Operand_Temp(p)));
                emit_irs.emplace_back(L_Store(AS_Operand_Temp(def), AS_Operand_Temp(p)));
                localVarMap.emplace(def->varname, p);
            }
        } else if (decl->kind == A_varDeclArrayKind) {
        if (decl->u.declArray->type->type == A_structTypeKind) {
            string structname = *decl->u.declArray->type->u.structType;
            Temp_temp* def = Temp_newtemp_struct_ptr(-1, structname);
            def->varname = *decl->u.declArray->id;
            args.push_back(def);
            localVarMap.emplace(def->varname, def);
        } else {
            Temp_temp* def = Temp_newtemp_int_ptr(-1);
            def->varname = *decl->u.declArray->id;
            args.push_back(def);
            localVarMap.emplace(def->varname, def);
        }
...
void ast2llvmVarDeclStmt(aA_varDeclStmt s) {
    if (s->kind == A_varDeclStmtType::A_varDeclKind) {
        ...
        if (s->u.varDecl->kind == A_varDeclType::A_varDeclScalarKind) {
            string id = *s->u.varDecl->u.declScalar->id;
            if (s->u.varDecl->u.declScalar->type->type == A_dataType::A_structTypeKind) {
                string structname = *s->u.varDecl->u.declScalar->type->u.structType;
                temp = Temp_newtemp_struct_ptr(0, structname);
                temp->varname = id;
            } else {
            ...
            int len = s->u.varDecl->u.declArray->len;
            string id = *s->u.varDecl->u.declArray->id;
            if (s->u.varDecl->u.declArray->type->type == A_dataType::A_structTypeKind) {
                string structname = *s->u.varDecl->u.declArray->type->u.structType;
                temp = Temp_newtemp_struct_ptr(len, structname);
                temp->varname = id;
            } else {
                temp = Temp_newtemp_int_ptr(len);
                temp->varname = id;
            }
        } else {
            assert(0);
        ...
        if (s->u.varDef->kind == A_varDefType::A_varDefScalarKind) {
            string id = *s->u.varDef->u.defScalar->id;
            if (s->u.varDef->u.defScalar->type->type == A_dataType::A_structTypeKind) {
                string structname = *s->u.varDef->u.defScalar->type->u.structType;
                auto temp = Temp_newtemp_struct_ptr(0, structname);
                temp->varname = id;
                localVarMap.emplace(temp->varname, temp);
                auto operand = AS_Operand_Temp(temp);
                emit_irs.push_back(L_Alloca(operand));
            ...
            string id = *s->u.varDef->u.defArray->id;
            if (s->u.varDef->u.defArray->type->type == A_dataType::A_structTypeKind) {
                string structname = *s->u.varDef->u.defArray->type->u.structType;
                auto temp = Temp_newtemp_struct_ptr(0, structname);
                temp->varname = id;
                localVarMap.emplace(temp->varname, temp);
                auto operand = AS_Operand_Temp(temp);
                ...
                int len = s->u.varDef->u.defArray->len;
...
void ast2llvmAssignStmt(aA_assignStmt s) {
    ...
    if (dst->kind == OperandKind::TEMP && dst->u.TEMP->type == TempType::INT_TEMP) {
...
{
    auto left_unit = ast2llvmExprUnit(c->left);
    auto right_unit = ast2llvmExprUnit(c->right);
    auto dst = AS_Operand_Temp(Temp_newtemp_int());
    switch (c->op) {
        case A_comOp::A_eq:
            emit_irs.emplace_back(L_Cmp(L_relopKind::T_eq, left_unit, right_unit, dst));
            break;
        case A_comOp::A_ge:
            emit_irs.emplace_back(L_Cmp(L_relopKind::T_ge, left_unit, right_unit, dst));
            break;
        case A_comOp::A_gt:
            emit_irs.emplace_back(L_Cmp(L_relopKind::T_gt, left_unit, right_unit, dst));
            break;
        case A_comOp::A_le:
            emit_irs.emplace_back(L_Cmp(L_relopKind::T_le, left_unit, right_unit, dst));
            break;
        case A_comOp::A_lt:
            emit_irs.emplace_back(L_Cmp(L_relopKind::T_lt, left_unit, right_unit, dst));
            break;
        case A_comOp::A_ne:
            emit_irs.emplace_back(L_Cmp(L_relopKind::T_ne, left_unit, right_unit, dst));
            break;
        default:
            assert(0);
    }
...
{
    auto left_expr = ast2llvmArithExpr(a->left);
    auto right_expr = ast2llvmArithExpr(a->right);
    auto dst = AS_Operand_Temp(Temp_newtemp_int());
    switch (a->op) {
        case A_arithBiOp::A_add:
            emit_irs.emplace_back(L_Binop(L_binopKind::T_plus, left_expr, right_expr, dst));
            break;
        case A_arithBiOp::A_div:
            emit_irs.emplace_back(L_Binop(L_binopKind::T_div, left_expr, right_expr, dst));
            break;
        case A_arithBiOp::A_mul:
            emit_irs.emplace_back(L_Binop(L_binopKind::T_mul, left_expr, right_expr, dst));
            break;
        case A_arithBiOp::A_sub:
            emit_irs.emplace_back(L_Binop(L_binopKind::T_minus, left_expr, right_expr, dst));
            break;
        default:
            assert(0);
            break;
...
AS_operand* ast2llvmArithUExpr(aA_arithUExpr a)
    {
    AS_operand *left_op = AS_Operand_Const(0);
    AS_operand *right_expr = ast2llvmExprUnit(a->expr);
    AS_operand *dst = AS_Operand_Temp(Temp_newtemp_int());
    emit_irs.emplace_back(L_Binop(L_binopKind::T_minus, left_op, right_expr, dst));
    return dst;
}
